...
async def create_region(payload: RegionCreate, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
    return await region_crud.create_region(conn, payload.region_name, payload.region_code)


@router.get("/", response_model=list[RegionOut])
async def list_regions(conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(get_current_user)):
    return await region_crud.get_all_regions(conn)


@router.get("/{region_id}", response_model=RegionOut)
async def get_region(region_id: UUID, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(get_current_user)):
...
async def update_region(region_id: UUID, payload: RegionUpdate, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
...
async def delete_region(region_id: UUID, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
