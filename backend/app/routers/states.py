...
async def create_state(payload: StateCreate, conn: asyncpg.Connection = Depends(get_db)):
    try:
...
async def get_state(state_id: UUID, conn: asyncpg.Connection = Depends(get_db)):
...
async def update_state(state_id: UUID, payload: StateUpdate, conn: asyncpg.Connection = Depends(get_db)):
...
async def delete_state(state_id: UUID, conn: asyncpg.Connection = Depends(get_db)):
