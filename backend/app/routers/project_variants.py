...
async def create_variant(payload: ProjectVariantCreate, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
    try:
...
async def list_variants(conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(get_current_user)):
    return await variant_crud.get_all_variants(conn)


@router.get("/{variant_id}", response_model=ProjectVariantOut)
async def get_variant(variant_id: UUID, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(get_current_user)):
...
async def update_variant(variant_id: UUID, payload: ProjectVariantUpdate, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
...
async def delete_variant(variant_id: UUID, conn: asyncpg.Connection = Depends(get_db), _: dict = Depends(require_admin)):
