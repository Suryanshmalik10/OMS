...
async def create_project(payload: ProjectCreate, conn: asyncpg.Connection = Depends(get_db)):
    try:
...
async def list_projects(conn: asyncpg.Connection = Depends(get_db)):
    return await project_crud.get_all_projects(conn)


@router.get("/{project_id}", response_model=ProjectOut)
async def get_project(project_id: UUID, conn: asyncpg.Connection = Depends(get_db)):
...
async def update_project(project_id: UUID, payload: ProjectUpdate, conn: asyncpg.Connection = Depends(get_db)):
...
async def delete_project(project_id: UUID, conn: asyncpg.Connection = Depends(get_db)):
